# Requirements Archive: v0.1 Core Engine
...
- [x] **API-01**: `XiangqiEngine` 类：`reset()`、`apply_move(move)`、`undo_move()`、`is_legal(move)`、`legal_moves()`、`is_check()`、`result()`
- [x] **API-02**: 走法执行后状态正确更新（board、turn、move_history、halfmove_clock）
- [x] **API-03**: FEN 解析与导出：`from_fen(fen)`、`to_fen()`
...
| END-01 | checkmate detection | Phase 3 | [x] |
| END-02 | stalemate detection | Phase 3 | [x] |
| END-03 | perpetual check draw | Phase 3 | [x] |
| END-04 | repetition draw | Phase 3 | [x] |
...
| API-04 | performance targets | Phase 4 | [x] |
| TEST-02 | pyffish cross-validate | Phase 4 | [x] |
| TEST-03 | boundary positions | Phase 4 | [x] |
| TEST-04 | special rules tests | Phase 4 | [x] |

**Coverage:** 31/31 satisfied (179 tests passing)

> **Note:** END-01..04, API-04, TEST-02..04 were completed after archive creation. Project-level PROJECT.md is authoritative; traceability table below reflects final state.
