...
- truth: "点击红色棋子后显示金色选择环"
  ...
  artifacts:
    - path: "src/xiangqi/ui/main.py"
      issue: "line 31: self.setCentralWidget(QXiangqiBoard()) - 缺少 engine 参数"
    - path: "src/xiangqi/ui/board.py"
      issue: "line 183-184: if self._engine is None: return - 没有 engine 时提前返回"
  missing:
    - "MainWindow 中创建 XiangqiEngine 实例"
    - "MainWindow 中创建 XiangqiState 实例"
    - "QXiangqiBoard 构造时传递 state 和 engine 参数"
  debug_session: ".planning/debug/click-no-response.md"
